...
function extendMaterial(BaseMaterial, cfg) {
    ...
    Object.entries(cfg.uniforms ?? {}).forEach(([key, value]) => {
        uniforms[key] = value !== null && typeof value === "object" && "value" in value
            ? value
            : { value };
    });
    ...
    for (const [include, code] of Object.entries(cfg.vertex ?? {})) {
        vert = vert.replace(include, `${include}\n${code}`);
    }

    for (const [include, code] of Object.entries(cfg.fragment ?? {})) {
        frag = frag.replace(include, `${include}\n${code}`);
    }
...
const MergedPlanes = forwardRef(({ material, width, count, height, spacing }, ref) => {
