export default function ContactMe() {
    return (
        <section id="Contact" className="contact--section" data-home-section>
            <div
                className="contact--section--container contact--section--intro scroll-reveal scroll-reveal--up"
                data-animate-on-scroll
            >
                <h2 className="skills--section--heading">Contact Me</h2>
                <p className="text-lg">
                    Feel free to reach out to me through any of these channels. I'm always open to discussing new projects, creative ideas, or opportunities to be part of your visions.
                </p>
            </div>
            <div
                className="contact--section--container scroll-reveal"
                data-animate-on-scroll
                style={{ "--reveal-delay": "0.08s" }}
            >
                <div className="contact--section--content">
                    <div className="contact--info--wrapper">
                        <div className="contact--info scroll-reveal scroll-reveal--left" data-animate-on-scroll style={{ "--reveal-delay": "0.12s" }}>
                            <div className="contact--icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path>
                                    <polyline points="22,6 12,13 2,6"></polyline>
                                </svg>
                            </div>
                            <div className="contact--text">
                                <h3>Email</h3>
                                <p>elliotnike@gmail.com</p>
                            </div>
                        </div>
                        <div className="contact--info scroll-reveal scroll-reveal--left" data-animate-on-scroll style={{ "--reveal-delay": "0.2s" }}>
                            <div className="contact--icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
                                    <circle cx="12" cy="10" r="3"></circle>
                                </svg>
                            </div>
                            <div className="contact--text">
                                <h3>Location</h3>
                                <p>Helsinki and Tampere, Finland</p>
                            </div>
                        </div>
                        <div className="contact--info scroll-reveal scroll-reveal--left" data-animate-on-scroll style={{ "--reveal-delay": "0.28s" }}>
                            <div className="contact--icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path>
                                </svg>
                            </div>
                            <div className="contact--text">
                                <h3>Phone</h3>
                                <p>+358 414410070</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    )
}
